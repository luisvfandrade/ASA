...
class Graph {
    ...
    public:
        ...
        void bfsLca(int flag, int node, int *visited, set<int> *parents, set<int> *lca);
...
void Graph::bfsLca(int flag, int node, int *visited, set<int> *parents, set<int> *lca = NULL) {
    queue<int> queue;
    ...
    while (!queue.empty()) {
        ...
        for (int j = 0; j < revList[i][2]; j++) {
            ...
            else if (flag == BFS_V2 && (*parents).find(revList[i][j]) != (*parents).end())
...
set<int> Graph::lca(int v1, int v2) {
    ...
    parentsV1.insert(v1);
    bfsLca(BFS_V1, v1, visited, &parentsV1);
    
    if (parentsV1.find(v2) != parentsV1.end())
        lca.insert(v2);
    else {
        memset(visited, 0, nodes * sizeof(*visited));
        bfsLca(BFS_V2, v2, visited, &parentsV1, &lca);
        memset(visited, 0, nodes * sizeof(*visited));
        for (set<int>::iterator i = lca.begin(); i != lca.end(); ++i)
            for (int j = 0; j < revList[*i][2]; j++)
                if (lca.find(revList[*i][j]) != lca.end())
                    bfsLca(BFS_REM, revList[*i][j], visited, &lca);
    }
    return lca;
}
